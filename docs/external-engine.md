...
every CPU core and a hash of half the machine's RAM — so the threads/memory sliders in the
Lichess engine manager have plenty of headroom; override with `--max-threads` / `--max-hash`.
The manual steps below explain exactly what it does.
